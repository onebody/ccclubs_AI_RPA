...
- [violation_query.py](file://CCC_RPA_API/app/browser/sub_tasks/violation_query.py)
- [violation_sync.py](file://CCC_RPA_API/app/browser/sub_tasks/violation_sync.py)
- [violation_transfer.py](file://CCC_RPA_API/app/browser/sub_tasks/violation_transfer.py)
...
**更新** 新增违章管理系统，扩展了浏览器自动化能力，支持从政府系统自动提取车辆违章数据并同步到外部SaaS平台，实现完整的多阶段自动化流程，包括违章查询、数据同步和违章转移等功能。
...
VIOLATION_SUBTASKS["违章子任务<br/>violation_query.py<br/>violation_sync.py<br/>violation_transfer.py"]
...
- [violation_query.py:1-1302](file://CCC_RPA_API/app/browser/sub_tasks/violation_query.py#L1-L1302)
- [violation_sync.py:1-1243](file://CCC_RPA_API/app/browser/sub_tasks/violation_sync.py#L1-L1243)
- [violation_transfer.py:1-802](file://CCC_RPA_API/app/browser/sub_tasks/violation_transfer.py#L1-L802)
...
- 子任务框架：SubTaskRegistry 管理多种业务子任务，包括合同录入、合同备案、违章查询、违章同步和违章转移。
- 前端交互：store 与组件根据 WebSocket 消息驱动 UI 状态流转。

**更新** 新增违章管理系统，支持违章查询、数据同步和违章转移三大核心功能，扩展了浏览器自动化能力。
...
- [violation_query.py:11-1302](file://CCC_RPA_API/app/browser/sub_tasks/violation_query.py#L11-L1302)
- [violation_sync.py:18-1243](file://CCC_RPA_API/app/browser/sub_tasks/violation_sync.py#L18-L1243)
- [violation_transfer.py:9-802](file://CCC_RPA_API/app/browser/sub_tasks/violation_transfer.py#L9-L802)
...
participant ST as "子任务<br/>contract_entry.py/filing.py/violation_*"
FE->>Store : 用户操作扫码完成/选择单位/取消
Store->>API : 发起后端接口扫码完成/选择单位/取消
API->>Exec : 触发任务执行
Exec->>SM : 获取/创建浏览器上下文
Exec->>SA : 登录检查/扫码/抓取/保活
SA->>ST : 执行子任务合同录入/备案/违章查询/同步/转移
ST-->>SA : 返回子任务结果
SA-->>Exec : 返回结果或状态
...
- [violation_query.py:79-126](file://CCC_RPA_API/app/browser/sub_tasks/violation_query.py#L79-L126)
- [violation_sync.py:75-183](file://CCC_RPA_API/app/browser/sub_tasks/violation_sync.py#L75-L183)
- [violation_transfer.py:35-127](file://CCC_RPA_API/app/browser/sub_tasks/violation_transfer.py#L35-L127)
...
- 子任务执行
  - 通过 SubTaskRegistry 支持多种业务子任务的动态调度，包括新增的违章管理系统。
...
PendingBiz -- 是 --> ExecSub["执行子任务占位<br/>合同/备案/违章"]
ExecSub --> Home["回到首页"]
Home --> KeepAlive
...
- 设计要点
  ...
  - 支持多种业务类型的子任务注册和动态调度。
  - 内置合同录入、合同备案、违章查询、违章同步和违章转移五大核心业务子任务。
- 关键功能
...
class ViolationQuerySubTask {
+SUB_TASK_TYPE
+IMAGE_DIR
+execute() dict
+_sync_all_violations(save_images) dict
+_query_by_plate(plate_number, plate_type, save_images) dict
+_search_plate(plate_number, plate_type) bool
+_extract_violation_list() list
+_fetch_all_details(violations, save_images) list
}
class ViolationSyncSubTask {
+SUB_TASK_TYPE
+CACHE_DIR
+IMAGE_DIR
+execute() dict
+_navigate_to_vehicle_list() bool
+_get_vehicle_list() list
+_query_vehicle_violations(vehicle, save_images) list
+_cache_data(vehicle, violations) str
+_batch_submit_to_saas(cached_data) dict
}
class ViolationTransferSubTask {
+SUB_TASK_TYPE
+execute() dict
+_get_transfer_data() dict
+_navigate_to_transfer_page() bool
+_get_transferable_violations() list
+_fill_driver_info(transfer_data) bool
+_submit_transfer() bool
+_verify_transfer() bool
}
SubTaskRegistry --> BaseSubTask
BaseSubTask <|-- ContractEntrySubTask
BaseSubTask <|-- FilingSubTask
BaseSubTask <|-- ViolationQuerySubTask
BaseSubTask <|-- ViolationSyncSubTask
BaseSubTask <|-- ViolationTransferSubTask
```
...
- [violation_transfer.py:9-802](file://CCC_RPA_API/app/browser/sub_tasks/violation_transfer.py#L9-L802)
- [base.py:8-121](file://CCC_RPA_API/app/browser/sub_tasks/base.py#L8-L121)
- [__init__.py:13-43](file://CCC_RPA_API/app/browser/sub_tasks/__init__.py#L13-L43)
...
### 违章查询子任务（ViolationQuerySubTask）
- 功能特性
  - 支持两种查询模式：全量同步所有违章记录和按车牌号精确查询。
  - 全量同步模式：自动翻页、提取详情、保存图片，支持批量处理。
  - 精确查询模式：通过车牌号和号牌种类进行精准定位。
  - 多策略导航：菜单hover、直接点击、URL直达等方式进入违章查询页面。
  - 智能解析：支持表格行、列表项、页面文本等多种数据源的违章信息提取。
- 高级功能
  - 图片截图：可选保存违章相关图片到 data/violation_images/ 目录。
  - 结果广播：实时向执行器报告执行进度和状态。
  - 结果保存：自动保存查询结果到 data/violation_results/ 目录。
  - 进度广播：支持附加数据字段的详细结果广播。

```mermaid
flowchart TD
Start(["违章查询开始"]) --> Mode{"查询模式"}
Mode -- 全量同步 --> SyncAll["全量同步模式"]
Mode -- 按车牌查询 --> QueryPlate["按车牌查询模式"]
SyncAll --> TriggerQuery["触发初始查询"]
TriggerQuery --> ExtractList["提取违章列表"]
ExtractList --> HasNext{"有下一页?"}
HasNext -- 是 --> ClickNext["点击下一页"]
ClickNext --> WaitReady["等待页面就绪"]
WaitReady --> ExtractList
HasNext -- 否 --> FetchDetails["批量提取详情"]
FetchDetails --> SaveImages["可选：保存图片"]
SaveImages --> BuildResult["构建结果并保存文件"]
BuildResult --> Broadcast["广播结果"]
QueryPlate --> SearchPlate["搜索指定车牌"]
SearchPlate --> ExtractList2["提取违章列表"]
ExtractList2 --> HasNext2{"有下一页?"}
HasNext2 -- 是 --> ClickNext2["点击下一页"]
ClickNext2 --> WaitReady2["等待页面就绪"]
WaitReady2 --> ExtractList2
HasNext2 -- 否 --> FetchDetails2["提取详情"]
FetchDetails2 --> SaveImages2["可选：保存图片"]
SaveImages2 --> BuildResult2["构建结果"]
BuildResult2 --> Broadcast2["广播结果"]
```

**图表来源**
- [violation_query.py:79-126](file://CCC_RPA_API/app/browser/sub_tasks/violation_query.py#L79-L126)
- [violation_query.py:131-179](file://CCC_RPA_API/app/browser/sub_tasks/violation_query.py#L131-L179)
- [violation_query.py:185-237](file://CCC_RPA_API/app/browser/sub_tasks/violation_query.py#L185-L237)
- [violation_query.py:239-347](file://CCC_RPA_API/app/browser/sub_tasks/violation_query.py#L239-L347)
- [violation_query.py:496-542](file://CCC_RPA_API/app/browser/sub_tasks/violation_query.py#L496-L542)
- [violation_query.py:574-683](file://CCC_RPA_API/app/browser/sub_tasks/violation_query.py#L574-L683)
- [violation_query.py:919-967](file://CCC_RPA_API/app/browser/sub_tasks/violation_query.py#L919-L967)
...
### 违章同步子任务（ViolationSyncSubTask）
- 功能特性
  - 完整的多阶段自动化流程：导航到租赁车管理页面 → 获取车辆列表 → 筛选有违章的车辆 → 逐车查询违章详情 → 本地缓存 → 批量提交到SaaS API。
  - 智能车辆筛选：支持只查询未处理违章的车辆，提高效率。
  - 本地缓存：将违章数据保存到 data/violation_sync_cache/ 目录，支持图片保存。
  - SaaS API集成：支持配置 saas_api_url 和 saas_api_key，批量提交到外部平台。
  - 多策略导航：菜单点击、JS查找、URL直达等方式进入车辆管理页面。
  - 翻页处理：自动检测和处理多页车辆列表和违章详情。
- 核心流程
  - 第一阶段：导航到租赁车管理页面并获取车辆列表
  - 第二阶段：逐车查询违章详情，支持图片截图
  - 第三阶段：本地缓存到JSON文件
  - 第四阶段：批量提交到SaaS API
  - 第五阶段：返回汇总结果
...
```mermaid
flowchart TD
Start(["违章同步开始"]) --> Navigate["导航到租赁车管理页面"]
Navigate --> GetVehicleList["获取车辆列表含翻页"]
GetVehicleList --> FilterVehicles["筛选有违章的车辆"]
FilterVehicles --> LoopVehicles{"遍历每辆车"}
LoopVehicles -- 是 --> QueryViolations["查询车辆违章详情"]
QueryViolations --> CacheData["本地缓存数据"]
CacheData --> WaitHuman["随机等待"]
WaitHuman --> LoopVehicles
LoopVehicles -- 否 --> LoadCached["加载所有缓存数据"]
LoadCached --> BatchSubmit["批量提交到SaaS API"]
BatchSubmit --> BuildResult["构建最终结果"]
BuildResult --> Broadcast["广播进度和结果"]
Broadcast --> End(["结束"])
```

**图表来源**
- [violation_sync.py:75-183](file://CCC_RPA_API/app/browser/sub_tasks/violation_sync.py#L75-L183)
- [violation_sync.py:188-290](file://CCC_RPA_API/app/browser/sub_tasks/violation_sync.py#L188-L290)
- [violation_sync.py:291-317](file://CCC_RPA_API/app/browser/sub_tasks/violation_sync.py#L291-L317)
- [violation_sync.py:474-527](file://CCC_RPA_API/app/browser/sub_tasks/violation_sync.py#L474-L527)
- [violation_sync.py:1024-1068](file://CCC_RPA_API/app/browser/sub_tasks/violation_sync.py#L1024-L1068)
- [violation_sync.py:1073-1156](file://CCC_RPA_API/app/browser/sub_tasks/violation_sync.py#L1073-L1156)
...
### 违章转移子任务（ViolationTransferSubTask）
- 功能特性
  - 支持将违章转移到承租驾驶人，实现完整的违章处理自动化。
  - 多策略导航：菜单点击、URL直达、JS查找等方式进入转移申请页面。
  - 可转移违章筛选：自动获取可转移的违章列表，支持未处理状态筛选。
  - 智能选择：支持按违章ID或默认选择第一条可转移违章。
  - 驾驶人信息填写：支持证件类型、证件号码等关键字段的自动化填写。
  - 提交验证：自动提交转移申请并验证结果。
- 核心流程
  - 获取转移数据 → 导航到转移页面 → 获取可转移违章 → 选择目标违章 → 填写驾驶人信息 → 提交申请 → 验证结果

**章节来源**
- [violation_transfer.py:35-127](file://CCC_RPA_API/app/browser/sub_tasks/violation_transfer.py#L35-L127)
- [violation_transfer.py:132-226](file://CCC_RPA_API/app/browser/sub_tasks/violation_transfer.py#L132-L226)
- [violation_transfer.py:232-308](file://CCC_RPA_API/app/browser/sub_tasks/violation_transfer.py#L232-L308)
- [violation_transfer.py:342-433](file://CCC_RPA_API/app/browser/sub_tasks/violation_transfer.py#L342-L433)
- [violation_transfer.py:438-579](file://CCC_RPA_API/app/browser/sub_tasks/violation_transfer.py#L438-L579)
- [violation_transfer.py:580-742](file://CCC_RPA_API/app/browser/sub_tasks/violation_transfer.py#L580-L742)
- [violation_transfer.py:748-802](file://CCC_RPA_API/app/browser/sub_tasks/violation_transfer.py#L748-L802)
...
- 控制流
  ...
  - 子任务执行：通过 SiteAutomation.execute_sub_task 调度具体业务处理，包括新增的违章管理系统。
  - 资源回收：关闭页面，更新任务状态与日志。
- 线程模型
...
DetectBiz --> |是| ExecSub["执行子任务<br/>合同/备案/违章"]
ExecSub --> KeepAlive
...
- 后端模块依赖
  ...
  - 新增的违章子任务依赖 BaseSubTask 基类，支持独立的业务流程。
- 数据模型
...
ST --> FI["FilingSubTask"]
ST --> VQ["ViolationQuerySubTask"]
ST --> VS["ViolationSyncSubTask"]
ST --> VT["ViolationTransferSubTask"]
Exec["Executor"] --> SM
Exec --> SA
...
- 违章查询失败
  - 现象：违章查询无法进入页面或数据提取失败。
  - 处理：检查导航策略是否成功；确认查询模式配置正确；查看截图定位问题。
  - 参考：[violation_query.py:1019-1092](file://CCC_RPA_API/app/browser/sub_tasks/violation_query.py#L1019-L1092)，[violation_query.py:1277-1302](file://CCC_RPA_API/app/browser/sub_tasks/violation_query.py#L1277-L1302)
- 违章同步失败
  - 现象：违章同步过程中断或SaaS API提交失败。
  - 处理：检查 saas_api_url 和 saas_api_key 配置；确认网络连接；查看缓存文件。
  - 参考：[violation_sync.py:1073-1156](file://CCC_RPA_API/app/browser/sub_tasks/violation_sync.py#L1073-L1156)，[violation_sync.py:1024-1068](file://CCC_RPA_API/app/browser/sub_tasks/violation_sync.py#L1024-L1068)
- 违章转移失败
  - 现象：违章转移流程中断或验证失败。
  - 处理：检查转移数据配置；确认驾驶人信息填写成功；查看提交后的页面状态。
  - 参考：[violation_transfer.py:150-226](file://CCC_RPA_API/app/browser/sub_tasks/violation_transfer.py#L150-L226)，[violation_transfer.py:438-579](file://CCC_RPA_API/app/browser/sub_tasks/violation_transfer.py#L438-L579)
- 子任务执行异常
  - 现象：子任务无法识别或执行失败。
  - 处理：确认子任务类型配置正确；检查 SubTaskRegistry 注册表；查看子任务日志。
  - 参考：[site_automation.py:738-750](file://CCC_RPA_API/app/browser/site_automation.py#L738-L750)，[__init__.py:13-43](file://CCC_RPA_API/app/browser/sub_tasks/__init__.py#L13-L43)
...
- [contract_entry.py:110-169](file://CCC_RPA_API/app/browser/sub_tasks/contract_entry.py#L110-L169)
- [contract_entry.py:310-394](file://CCC_RPA_API/app/browser/sub_tasks/contract_entry.py#L310-L394)
- [violation_query.py:1019-1092](file://CCC_RPA_API/app/browser/sub_tasks/violation_query.py#L1019-L1092)
- [violation_query.py:1277-1302](file://CCC_RPA_API/app/browser/sub_tasks/violation_query.py#L1277-L1302)
- [violation_sync.py:1073-1156](file://CCC_RPA_API/app/browser/sub_tasks/violation_sync.py#L1073-L1156)
- [violation_sync.py:1024-1068](file://CCC_RPA_API/app/browser/sub_tasks/violation_sync.py#L1024-L1068)
- [violation_transfer.py:150-226](file://CCC_RPA_API/app/browser/sub_tasks/violation_transfer.py#L150-L226)
- [violation_transfer.py:438-579](file://CCC_RPA_API/app/browser/sub_tasks/violation_transfer.py#L438-L579)
- [site_automation.py:738-750](file://CCC_RPA_API/app/browser/site_automation.py#L738-L750)
- [__init__.py:13-43](file://CCC_RPA_API/app/browser/sub_tasks/__init__.py#L13-L43)

## 结论
本系统通过专用线程隔离 Playwright、完善的会话管理与状态持久化、人性化行为模拟与智能等待机制，实现了稳定可靠的浏览器自动化流程。新增的违章管理系统进一步增强了系统的扩展性，支持违章查询、数据同步和违章转移三大核心业务场景的自动化处理，包括完整的多阶段自动化流程和SaaS平台集成。前后端通过 WebSocket 实时通信，前端以清晰的状态机驱动用户交互，整体具备良好的扩展性与可维护性。建议后续在代理池与指纹伪装方面进一步增强，以应对更严格的反爬策略。
...
- 子任务执行
  - 子任务注册表：[__init__.py:13-43](file://CCC_RPA_API/app/browser/sub_tasks/__init__.py#L13-L43)
  ...
  - 违章查询主流程：[violation_query.py:79-126](file://CCC_RPA_API/app/browser/sub_tasks/violation_query.py#L79-L126)
  - 违章查询导航：[violation_query.py:1019-1092](file://CCC_RPA_API/app/browser/sub_tasks/violation_query.py#L1019-L1092)
  - 违章同步主流程：[violation_sync.py:75-183](file://CCC_RPA_API/app/browser/sub_tasks/violation_sync.py#L75-L183)
  - 违章转移主流程：[violation_transfer.py:35-127](file://CCC_RPA_API/app/browser/sub_tasks/violation_transfer.py#L35-L127)
- 执行器与 WebSocket
