...
**更新** 本版本新增了'合同录入'子任务功能，扩展了任务编辑功能的可用子任务类型，为用户提供更全面的自动化任务选项。
...
**更新** 任务编辑页现已支持五种子任务类型：合同备案、合同录入、违章查询、合同调整、违章转移，为用户提供了更丰富的自动化任务选择。
...
- [TaskEditPage.vue:86-95](file://CCC-BrowserV4/frontend/src/pages/TaskEditPage.vue#L86-L95)
...
- 子任务选项：支持合同备案、合同录入、违章查询、合同调整、违章转移五种子任务类型的选择。

**更新** 新增'合同录入'子任务选项，扩展了任务编辑功能的可用子任务类型。该功能允许用户创建专门用于在122.gov.cn平台上自动完成租赁合同录入的自动化任务。

```mermaid
flowchart TD
Start(["打开任务编辑页"]) --> Mode{"编辑模式？"}
Mode --> |是| Load["加载现有任务数据"]
Mode --> |否| Init["初始化新任务表单"]
Load --> EditForm["显示编辑表单"]
Init --> NewForm["显示新建表单"]
EditForm --> SubTasks["子任务选择"]
NewForm --> SubTasks
SubTasks --> Options["合同备案/合同录入/违章查询/合同调整/违章转移"]
Options --> Save["保存任务"]
Save --> Success["跳转任务列表"]
```
...
图表来源
- [TaskEditPage.vue:86-95](file://CCC-BrowserV4/frontend/src/pages/TaskEditPage.vue#L86-L95)
- [TaskEditPage.vue:224-256](file://CCC-BrowserV4/frontend/src/pages/TaskEditPage.vue#L224-L256)

章节来源
- [TaskEditPage.vue:1-285](file://CCC-BrowserV4/frontend/src/pages/TaskEditPage.vue#L1-L285)
...
**更新** 新增的'合同录入'子任务功能进一步增强了系统的自动化能力，为用户提供了更全面的任务类型选择，特别是在车辆租赁合同管理领域的自动化处理能力。
...
- [main.css:1-27](file://CCC-BrowserV4/frontend/src/assets/styles/main.css#L1-L27)
- [TaskEditPage.vue:86-95](file://CCC-BrowserV4/frontend/src/pages/TaskEditPage.vue#L86-L95)
- [contract_entry.py:10-11](file://CCC_RPA_API/app/browser/sub_tasks/contract_entry.py#L10-L11)